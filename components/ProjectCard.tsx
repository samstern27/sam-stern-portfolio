...
export default function ProjectCard({
  ...
  // Handles clicking the card: opens the primary project URL in a new tab
  ...
  return (
    <div className="project-card">
      <button
        className="flex md:flex-row glowing-border w-90 sm:w-120 md:w-full h-130 sm:h-140 md:h-70 flex-col px-3.5 sm:px-0 pt-4 sm:pt-0 md:hover:scale-103 duration-500 sm:cursor-pointer"
        onClick={() => handleCardClick(primaryUrl)}
        tabIndex={0}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") handleCardClick(primaryUrl);
        }}
        aria-label={
          name === "Portfolio"
            ? `GitHub repository for ${name}, opens in a new tab`
            : `Vercel deployment of ${name}, opens in a new tab`
        }
      >
        {/* Image of website */}
        <div
        ...
        >
          ...
          <div className="flex flex-row md:flex-col justify-center gap-5 items-center my-5">
            <a
              ...
              title="Github"
              aria-label={`GitHub repository for ${name}, opens in a new tab`}
            >
              <span
                className="
            ...
            {/* Logic to exclude Vercel link on portfolio website */}
            {primaryUrl !== github ? (
              <a
                href={vercel}
                target="_blank"
                ...
                className="relative flex group"
                title="Vercel"
                aria-label={`Vercel deployment of ${name}, opens in a new tab`}
              >
                <Image
                  src="/vercel.svg"
                  width={683}
